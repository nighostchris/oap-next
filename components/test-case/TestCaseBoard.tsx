...
import { DropBoard, StatelessNewTest } from './DropBoard';
import Assertions from './Assertions';
import LogicStatements from './LogicStatements';
import { StatelessDataInput } from './DataInput';

const TestCaseContext = React.createContext({});

const testsReducer = (state: any, action: any) => {
  switch(action.type) {
    case 'ADD_TEST':
      return {
        ...state,
...
const TestCaseBoard: React.FunctionComponent = () => {
  ...
  return (
    <TestCaseContext.Provider value={{ testsState, testsDispatch }}>
          ...
          >
            {
              testsState.tests.map((test: any) => (
                <DropBoard testCaseName={test.name} />
              ))
            }
            <i
              ref={dropBin}
